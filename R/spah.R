...
#'@importFrom magrittr "%>%"
#'
#'@examples luminex_spah("/Users/phoebelam/Desktop/spah")
#'
#'@export
luminex_spah <- function (path) {
  ...
  for (f in filenames) {
    ...
    dat[start:end, 1:6] -> mfi
    
    colnames(mfi) <-c("location", "id", "il8", "il1b", "il6", "tnfa")
    ...
    tidyr::pivot_wider(dplyr::select(mfi, id, il8:tnfa, dup), 
                names_from = dup,
                values_from = c(il8, il1b, il6, tnfa)) %>%
      dplyr::rename_at(vars(il8_1:tnfa_2),
    ...
    mfi %>%
      dplyr::mutate_at(vars(il8_1_mfi:tnfa_2_mfi),
                list(~as.numeric(as.character(.)))) %>%
      dplyr::rowwise() %>%
      dplyr::mutate(il8_mfi_cv = abs(round(cv.p(c(il8_1_mfi, il8_2_mfi)),5)),
             il1b_mfi_cv = abs(round(cv.p(c(il1b_1_mfi, il1b_2_mfi)),5)),
    ...
    dat[start:end, 1:6] -> conc
    
    colnames(conc) <-c("location", "id", "il8", "il1b", "il6", "tnfa")
    ...
    tidyr::pivot_wider(dplyr::select(conc, id, il8:tnfa, dup), 
                names_from = dup,
                values_from = c(il8, il1b, il6, tnfa)) %>%
      dplyr::rename_at(vars(il8_1:tnfa_2),
    ...
    dat[start:end, 1:5] -> cv
    
    colnames(cv) <-c("id", "il8", "il1b", "il6", "tnfa")
    ...
    dat[start:end, 1:5] -> mfim
    
    colnames(mfim) <-c("id", "il8", "il1b", "il6", "tnfa")
    ...
    dat[start:end, 1:5] -> concm
    
    colnames(concm) <-c("id", "il8", "il1b", "il6", "tnfa")
    ...
    pecan::mergethem(mfi, conc, mfim, concm, cv) %>%
      dplyr::select(., id, contains("il8"), contains("il1b"), contains("il6"), contains("tnfa")) -> together
  ...
  for (f in filenames) {
    ...
    tidyr::pivot_wider(dplyr::select(conc, id, il8:tnfa, dup), 
                       names_from = dup,
                       values_from = c(il8, il1b, il6, tnfa)) %>%
      dplyr::rename_at(dplyr::vars(il8_1:tnfa_2),
                       list(~paste(., "_conc", sep=""))) -> conc
    ...
    cv %>%
      dplyr::rename_at(dplyr::vars(il8:tnfa),
                       list(~paste(., "_conc_cv", sep=""))) -> cv
  ...
  consol %>%
    ...
    dplyr::mutate_at(dplyr::vars(il8_conc_cv, il6_conc_cv, tnfa_conc_cv, il1b_conc_cv,
                          il8_mfi_cv, il6_mfi_cv, tnfa_mfi_cv, il1b_mfi_cv),
                     list(~as.numeric(as.character(.)))) -> consol
  
  consol %>%
    dplyr::select(., id,
                  il8_mfi_cv, il1b_mfi_cv, il6_mfi_cv, tnfa_mfi_cv,
                  il8_1_mfi,il8_2_mfi,il8_1_conc,il8_2_conc,il8_avg_mfi,
                  il8_avg_conc, il1b_1_mfi,il1b_2_mfi,
                  il1b_1_conc,il1b_2_conc,il1b_avg_mfi,il1b_avg_conc,
  ...
  consol %>%
    dplyr::filter(if_any(c(il8_mfi_cv, il6_mfi_cv, tnfa_mfi_cv, il1b_mfi_cv), ~ abs(.)> 10)) %>% 
    ...
    dplyr::mutate(us = case_when(grepl("US", id)==T~ 1,
                                 TRUE~0)) %>%
    mutate_at(dplyr::vars(il8_mfi_cv:tnfa_2_mfi),
  ...
  highcv %>%
    ...
    dplyr::select(., id, `repeat US as well`, decision, `reason for repeat`, `decision made by`,
                  il8_mfi_cv, il1b_mfi_cv, il6_mfi_cv, tnfa_mfi_cv,
                  il8_1_mfi,il8_2_mfi,il8_1_conc,il8_2_conc,
  ...
  stanconsol %>%
    dplyr::mutate_all(~gsub("NaN|#N/A|N/A", "", .)) %>%
    dplyr::mutate_at(vars(il8_conc_cv, il6_conc_cv, tnfa_conc_cv, il1b_conc_cv,
                          il8_mfi_cv, il6_mfi_cv, tnfa_mfi_cv, il1b_mfi_cv),
                     list(~as.numeric(as.character(.)))) -> stanconsol
  
  stanconsol %>%
    dplyr::select(., id,il8_1_mfi,il8_2_mfi,il8_1_conc,il8_2_conc,il8_avg_mfi,il8_avg_conc,il8_mfi_cv,
                  il8_conc_cv,il1b_1_mfi,il1b_2_mfi,il1b_1_conc,il1b_2_conc,il1b_avg_mfi,il1b_avg_conc,
  ...
  if (file.exists(paste(path, "/consolidated", sep=""))){
    ...
    openxlsx::write.xlsx(list("for master" = consol,
                              "high cv only" = highcv,
                              "standards & controls" = stanconsol), paste(path, "/consolidated/SPAH consolidated.xlsx", sep=""), rowNames=F)
