#'@importFrom magrittr "%>%"
#'@export
sleeplog_old <- function(path, id, Study, visit, daylight = F) {

  # troubleshoot
  # file <- read.csv("/Users/phoebelam/Desktop/sleep/MHS/MHS V2 Daily Diary Day 8_May 1, 2023_11.24.csv", header = T)
  # day8check = 8
  # id = 1193
  ...
  for (f in filenames) { # do not run this line when we troubleshooting
    ...
    if (any (file$goodid == id, na.rm=T) == TRUE) { #skip for troubleshooting
      ...
      if (day8check != 8) {
        ...
        other <- readRDS (paste(path, "/otherlog.rds", sep=""))
  ...
  if (Study == "OTR" & visit == 1)  {
    ...
    track <- openxlsx::read.xlsx (paste(path, "/MHS/MHS Actigraphy Tracking.xlsx", sep=""), startRow = 3, sheet = "Mentor V2", detectDates=T)
    ...
    track <- openxlsx::read.xlsx (paste(path, "/MHS/MHS Actigraphy Tracking.xlsx", sep=""), startRow = 3, sheet = "Mentee V2", detectDates=T)
  }
  ...
  if (nrow (log) > 0) {
   ...
   saveRDS (trackl, paste(path, "/track.rds", sep=""))
    
    
    log <- readRDS (paste(path, "/sleeplog.rds", sep="")) [-1,-1]
    other <- readRDS (paste(path, "/otherlog.rds", sep="")) [-1,-1]
    track <- readRDS (paste(path, "/track.rds", sep=""))
    ...
    if(length(which(!is.na(merge2$qualtrics_day.x))) ==8) {
      merge2 %>%
    ...
    xlsx::write.xlsx(tmerge2, paste (path, "/", Study," V", visit, " Sleep Log ", id, ".xlsx", sep =""), row.names = TRUE, col.names = FALSE, showNA = FALSE)
